...
const NetworkObj_1 = require("./schema/NetworkObj");
...
class GameRoom extends core_1.Room {
    constructor() {
        super(...arguments);
        this.state = new GameRoomState_1.GameRoomState();
        this.maxClients = 4;
        this.patchRate = 25;
    }
    // 방 생성
...
//#endregion
//#region Room Control
function SetRoomControlListener(room) {
    // 강퇴
    room.onMessage("Kick", (client, message) => {
    ...
    // 방 잠금
    room.onMessage("Lock", (client, message) => {
        if (client.sessionId !== room.state.hostId) {
    ...
    // 방 잠금 해제
    room.onMessage("Unlock", (client, message) => {
        if (client.sessionId !== room.state.hostId) {
    ...
    // 방 메타데이터 설정
    room.onMessage("Metadata-Set", (client, message) => {
        if (client.sessionId !== room.state.hostId) {
    ...
    // 방 메타데이터 가져오기
    room.onMessage("Metadata-Get", (client, message) => {
        client.send("metadata", room.metadata);
    });
}
//#endregion
//#region RPC
function SetRPCListener(room) {
...
function SetNetworkObjectListener(room) {
    // 네트워크 오브젝트 생성
    room.onMessage("Create", (client, message) => {
        ...
        room.state.Objects.set(objectId, new NetworkObj_1.NetworkObj);
...
//#endregion
//#region Authority
function SetAuthorityListener(room) {
